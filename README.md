...
- **Board room inbox** — when a delegated task reaches a terminal state, its coordinating AI employee **emails** the Board the final deliverable (✅ completed or ⚠️ needs-attention with the reason). The Board room has an **Inbox** tab alongside **Approvals**; unread mail shows on the sidebar badge, and each email carries the Copy / Add-to-knowledge actions.
