...
- **Projects with milestones, review gates & feedback branches** — bigger, longer initiatives. A project manager (default: the CEO) plans a project into an ordered set of **milestones**, each with a **gate**: an `auto` gate flows straight through when its tasks finish, while a **review gate** pauses the project for the Board to **approve or request changes**. Milestones are expanded into tasks only when they become active, so feedback and results from earlier gates shape the planning of later ones. **Your feedback becomes a formal branch step**: requesting changes at a gate (or giving feedback on any completed milestone or the whole project) inserts a new, clearly-marked **branch milestone** into the plan that addresses the feedback and comes back for review — feedback is always a visible step, never a silent rerun. When the project finishes, the manager writes a **comprehensive, sectioned report** — a detailed section per milestone plus an executive overview, assembled into one document — so nothing accomplished is left out. You can add tasks to a project at any time.
...
- **Recommends its own work** — once the company has **learned enough** (a chief AI employee plus a substantial knowledge base), the CEO proposes the highest-value next **projects and deliverables**, grounded in what the org knows. Recommendations land in the **Board room** as 💡 proposals with a rationale; approving one spins up a real project (planned into milestones) or a deliverable task that runs autonomously — dismiss the rest. They refresh automatically after work completes and on demand via **Suggest next steps** (threshold tunable with `LOOMAI_RECOMMEND_MIN_DOCS`).
