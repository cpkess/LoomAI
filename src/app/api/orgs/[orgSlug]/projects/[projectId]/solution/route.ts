  ...
  try {
    ...
    return Response.json({
      solution: {
        ...
        mergedFrom: Array.isArray(s.mergedFrom) ? s.mergedFrom : [],
        error: s.error,
      ...
      rounds: all.map((r) => ({
        ...
        mergedFrom: Array.isArray(r.mergedFrom) ? r.mergedFrom : [],
...
export async function POST(req: Request, { params }: { params: Promise<{ orgSlug: string; projectId: string }> }) {
  try {
    ...
    // Comparing rounds: pool their evidence and reconcile their conclusions.
    const compare: string[] = Array.isArray(body?.compare) ? body.compare.filter((x: unknown) => typeof x === "string") : [];
    if (compare.length > 0) {
      if (compare.length < 2) return Response.json({ error: "Pick at least two rounds to compare" }, { status: 400 });
      if (compare.length > 4) return Response.json({ error: "Compare at most four rounds at once" }, { status: 400 });
      try {
        const id = await compareSolutions(projectId, ctx.org.id, ctx.user.id, compare, direction);
        return Response.json({ solution: { id } }, { status: 201 });
      } catch (err) {
        return Response.json({ error: err instanceof Error ? err.message : "Could not compare" }, { status: 400 });
      }
    }

    // Forking from a named round, so a dead end doesn't trap the work.
    const from = typeof body?.from === "string" ? body.from : undefined;

    const id = direction
      ? await continueSolution(projectId, ctx.org.id, ctx.user.id, direction, research, from)
